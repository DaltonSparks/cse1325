...
void Mainwin::on_new_order_click() {
	...
	std::ostringstream oss;
	{
		...
		c_type.set_active(store->customers()-1);
		dialog.get_content_area()->add(c_type);
		...
		ordernum = store->add_order(store->customer(c_type.get_active_row_number()));
	...
	//products
	{
		...
		//spin button
		Gtk::HBox Obox;
		Gtk::Label q{"Quantity"};
		Obox.add(q);
		Gtk::SpinButton spin;
		spin.set_range(1.0, 99.0);
		spin.set_increments(1.0, 10.0);
		Obox.add(spin);
		dialog.get_content_area()->add(Obox);
		...
		while(true) {
			...
			if(dialog.run() == 1)
				store->add_item(ordernum, store->product(c_type.get_active_row_number()), static_cast<int>(spin.get_value()));
			else
				break;
		}
	}  
