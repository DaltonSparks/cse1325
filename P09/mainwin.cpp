...
void Mainwin::on_new_customer_click() {
	...
	Gtk::Label l_p{"Phone"};
	Gtk::Entry e_p;
	grid.attach(l_p, 0, 1, 1, 1);
	grid.attach(e_p, 1, 1, 2, 1);

	Gtk::Label l_e{"Email"};
	Gtk::Entry e_e;
	grid.attach(l_e, 0, 2, 1, 1);
	grid.attach(e_e, 1, 2, 2, 1);
	...
	std::string Cname = e_name.get_text();
	std::string Cphone = e_p.get_text();
	std::string Cemail = e_e.get_text();
